...
class DebtsAdapter(private val recordList: MutableList<DebtsList>) :
    RecyclerView.Adapter<DebtsAdapter.RecordViewHolder>() {

    inner class RecordViewHolder(private val binding: DebtsRecordsItemBinding) :
                ...
                override fun onResponse(call: Call<Debtmark>, response: Response<Debtmark>) {
                    if (response.isSuccessful) {
                        binding.resolveButton.apply {
                            text = "Resolved"
                            setTextColor(binding.root.context.resources.getColor(R.color.black))
                            setBackgroundColor(binding.root.context.resources.getColor(R.color.green))
                            isEnabled = false
                        }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecordViewHolder {
        val binding = DebtsRecordsItemBinding.inflate(LayoutInflater.from(parent.context), parent, false)
